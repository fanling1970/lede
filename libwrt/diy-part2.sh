...
# ====================================================================
# 6. 修复 luci-app-quickstart 首页CPU温度显示0℃
#    适配 JDCloud RE-CS-02 (ipq60xx)，CPU温度对应 thermal_zone4 (cpu-thermal)
# ====================================================================
echo "修复 QuickStart 首页温度显示..."

QUICKSTART_JS_PATH="feeds/nas_luci/luci-app-quickstart/htdocs/luci-static/quickstart"

# 修正前端JS文件：硬编码thermal_zone0 → thermal_zone4
if [ -d "$QUICKSTART_JS_PATH" ]; then
    sed -i 's/thermal_zone0/thermal_zone4/g' "$QUICKSTART_JS_PATH/index.js"
    sed -i 's/thermal_zone0/thermal_zone4/g' "$QUICKSTART_JS_PATH/vendor.js"
    echo "  ✓ 前端JS温度分区已修正"
fi

# 修正所有Lua后端文件中的温度读取路径
find feeds/nas_luci -name "*.lua" -type f 2>/dev/null | while read lua_file; do
    if grep -q "thermal_zone0" "$lua_file"; then
        sed -i 's/thermal_zone0/thermal_zone4/g' "$lua_file"
        echo "  ✓ 后端$(basename "$lua_file")温度分区已修正"
    fi
done
...
echo "5. QuickStart温度修复: ✓ (适配ipq60xx CPU分区)"
echo "=========================================="
