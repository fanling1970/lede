...
# 修改 wifi 无线名称
sed -i "s/ImmortalWrt/JDC_AX6600/g" package/network/config/wifi-scripts/files/lib/wifi/mac80211.uc

# 修改 wifi 无线名称 & 密码
sed -i "s/BASE_SSID='.*'/BASE_SSID='JDC_AX6600'/g" target/linux/qualcommax/base-files/etc/uci-defaults/990_set-wireless.sh
sed -i "s/BASE_WORD='.*'/BASE_WORD='BUZHIDAOWA'/g" target/linux/qualcommax/base-files/etc/uci-defaults/990_set-wireless.sh
